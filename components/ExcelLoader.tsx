...
export const ExcelLoader: React.FC<ExcelLoaderProps> = ({ onFileLoaded, isLoaded, fileName, isLoading }) => {
      ...
      <div className="flex flex-col items-center justify-center space-y-4">
        ...
        <div className="text-center">
          <h2 className="text-xl font-bold text-white">
            {isLoading ? 'Cargando Base de Datos...' : isLoaded ? 'Catálogo Vinculado' : 'Vincular Base de Datos Excel'}
          </h2>
          <p className="text-slate-400 text-sm mt-1">
            {isLoading ? 'Buscando LISTADO_PELIS.xlsx...' : isLoaded ? `Archivo actual: ${fileName}` : 'Selecciona tu archivo de películas para comenzar'}
          </p>
        </div>
